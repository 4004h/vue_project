<template>
  <div class="son">
    <!-- 내용을 추가하세요 -->
    <p>Son01 : {{ mag }} {{  }} </p>
    <Gson11 ref="gson11" />
    <Gson12 ref="gson12" @event_report="receiveEvent"/>

    <input type="text" v-model="sonmag" >
    <button @click="clickSon">버튼01</button>
    <button @click="clickSon2">버튼02</button>
    <button @click="clickSon3">버튼03</button>
  </div>
</template>

<script>
import Gson11 from './Gson11';
import Gson12 from './Gson12';

export default {
  name: 'Son-01',
  props: {
    // 문자열 타입의 prop 예시
    //sampleString: {
    //  type: String,
    //  default: ''
    //},
    // 숫자 타입의 prop 예시
    //sampleNumber: {
    //  type: Number,
    //  default: 0
    //},
    // 배열 타입의 prop 예시
    //sampleArray: {
    //  type: Array,
    //  default: () => []
    //},
    // 객체 타입의 prop 예시
    //sampleObject: {
    //  type: Object,
    //  default: () => ({})
    //}
  },
  components: {
    // 추가적으로 사용할 컴포넌트들을 등록합니다.
    Gson11, Gson12
  },
  data() {
    return {
      // 컴포넌트의 데이터를 초기화합니다.
      mag : '',
      sonmag : '',
    };
  },
  watch: {
    // sample1() {
    //   console.log('');
    // }
    // 데이터를 감시하고 처리할 로직을 작성합니다.
  },
  computed: {
    message() {
      console.log("fu")
      // console.log(this.$refs.gson11.gsmsg);
      return 0
      
    }
    // sample2() {
    //   return '';
    // }
    // 필요한 계산된 속성을 정의합니다.
  },
  methods: {
    clickSon() {
      this.mag = 'Son01 버튼 누름'
      this.$refs.gson11.clickMsg()
    },
    clickSon2() {
      this.mag = 'Son01 버튼 누름'
      this.$refs.gson12.clickMsg("son 명령")
    },
    clickSon3() {
      this.mag = 'Son01 버튼 누름'
      this.$refs.gson11.msg = this.sonmag 
      this.$refs.gson12.msg = this.sonmag
    },
    testOrder(i,v) {
      this.$refs.gson11.sonOrder(i,v)
      this.$refs.gson12.sonOrder(i,v)
    },

    receiveEvent(data) {
      alert(data)
    },

    // sample3() {
    //   return '';
    // }
    // 컴포넌트에서 사용할 메서드를 정의합니다.
  },
  setup() {
    // Vue 3 Composition API의 setup 함수에서 추가적인 로직을 처리할 수 있습니다.
  },
  created() {
    // 컴포넌트가 생성될 때 실행될 로직을 작성합니다.
  },
  mounted() {
    // 컴포넌트가 DOM에 마운트된 직후 실행될 로직을 작성합니다.
  },
  unmounted() {
    // 컴포넌트가 파괴되기 전 실행될 로직을 작성합니다.
  }
};
</script>

<style scoped>
/* 스타일을 추가하세요 */
</style>